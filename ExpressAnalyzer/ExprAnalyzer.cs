...
namespace ExpressAnalyzer_2
{
    using T = Double;   // Generic T does not support operators, and no such "where T : IArithmatcOperators"
    class ExprAnalyzer
    {
        #region construction
        public ExprAnalyzer()
        ...
        public ExprAnalyzer(string expr, Dictionary<string, T> vars)
        {
            this.expr = expr;
            this.Variables = vars;
        }
        ...
        public void SetVariableTable(string variableTable)
        {
            this.Variables = this.Variables ?? new Dictionary<string, T>();
